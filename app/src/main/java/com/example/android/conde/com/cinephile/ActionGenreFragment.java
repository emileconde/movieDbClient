...
public class ActionGenreFragment extends Fragment implements ItemClickListener {
    private static final String TAG = "ActionGenreFragment";
    private RecyclerView mRecyclerView;
    private MovieGenreAdapter mMovieGenreAdapter;
    private List<Movie> mActionMovieList;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        mRecyclerView = view.findViewById(R.id.rv_action_genre);
        mProgressBar = view.findViewById(R.id.progress_bar_action_genre);
        showProgressBar();
        mActionMovieList = new ArrayList<>();
        mMovieGenreAdapter = new MovieGenreAdapter(getContext(), mActionMovieList, this);
    ...
    private void subscribeObservers(){
        mGenreFragmentsViewModel.getActionMovieGenreList().observe(this, new Observer<List<Movie>>() {
            @Override
            public void onChanged(List<Movie> movies) {
                if (movies != null) {
                    if (mActionMovieList != null && mActionMovieList.size() > 0)
                        mActionMovieList.clear();
                    mActionMovieList.addAll(movies);
