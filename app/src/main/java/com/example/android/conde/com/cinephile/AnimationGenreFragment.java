...
public class AnimationGenreFragment extends Fragment implements ItemClickListener {

    private RecyclerView mRecyclerView;
    private MovieGenreAdapter mMovieGenreAdapter;
    private List<Movie> mAnimationMovieList;
    private GenreFragmentsViewModel mGenreFragmentsViewModel;
    private ProgressBar mProgressBar;

    public AnimationGenreFragment(){}

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
                             @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_animation_genre, container, false);
        mGenreFragmentsViewModel = ViewModelProviders.of(this).get(GenreFragmentsViewModel.class);
        mRecyclerView = view.findViewById(R.id.rv_animation_genre);
        mProgressBar = view.findViewById(R.id.progress_bar_animation_genre);
        mAnimationMovieList = new ArrayList<>();
        showProgressBar();
        mMovieGenreAdapter = new MovieGenreAdapter(getContext(), mAnimationMovieList, this);
    ...
    public void makeQueries(){
        mGenreFragmentsViewModel.queryMoviesByGenre(Constants.API_KEY,
                ...
                false,
                1,
                Constants.GENRE_ANIMATION);
    }



    private void subscribeObservers(){
        mGenreFragmentsViewModel.getAnimationMovieGenreList().observe(this, new Observer<List<Movie>>() {
            @Override
            public void onChanged(List<Movie> movies) {
                if (mAnimationMovieList != null && mAnimationMovieList.size() > 0 )
                    mAnimationMovieList.clear();
                mAnimationMovieList.addAll(movies);
