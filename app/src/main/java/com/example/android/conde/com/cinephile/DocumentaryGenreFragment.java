...
public class DocumentaryGenreFragment extends Fragment implements ItemClickListener {

    private RecyclerView mRecyclerView;
    private MovieGenreAdapter mMovieGenreAdapter;
    private List<Movie> mDocumentaryMovieList;
    private GenreFragmentsViewModel mGenreFragmentsViewModel;
    private ProgressBar mProgressBar;

    public DocumentaryGenreFragment(){}

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
                             @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_documentary_genre, container, false);
        mGenreFragmentsViewModel = ViewModelProviders.of(this).get(GenreFragmentsViewModel.class);
        mRecyclerView = view.findViewById(R.id.rv_documentary_genre);
        mProgressBar = view.findViewById(R.id.progress_bar_documentary_genre);
        showProgressBar();
        mDocumentaryMovieList = new ArrayList<>();
        mMovieGenreAdapter = new MovieGenreAdapter(getContext(), mDocumentaryMovieList, this);
    ...
    public void makeQueries(){
        mGenreFragmentsViewModel.queryMoviesByGenre(Constants.API_KEY,
                ...
                false,
                1,
                Constants.GENRE_DOCUMENTARY);
    }



    private void subscribeObservers(){
        mGenreFragmentsViewModel.getDocumentaryMovieGenreList().observe(this, new Observer<List<Movie>>() {
            @Override
            public void onChanged(List<Movie> movies) {
                if (mDocumentaryMovieList != null && mDocumentaryMovieList.size() > 0 )
                    mDocumentaryMovieList.clear();
                mDocumentaryMovieList.addAll(movies);
